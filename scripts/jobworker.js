...
function FlipImageData(e, t, s) {
    const r = 4 * t, o = new Uint8Array(r), n = e.buffer;
    for (let e = 0, t = Math.floor(s / 2); e < t; ++e) {
        const a = s - e - 1, l = new Uint8Array(n, e * r, r), f = new Uint8Array(n, a * r, r);
...
function SendError(e, t) {
    e || self.outputPort.postMessage({"type": "error", "jobId": self.activeJobId, "error": t.toString()}), SendDone()
}

function SendResult(e, t) {
    if (!e) {
        const s = t.transferables || [];
        self.outputPort.postMessage({"type": "result", "jobId": self.activeJobId, "result": t.result}, s)
    }
    SendDone()
}
...
function SendProgress(e) {
    self.outputPort.postMessage({"type": "progress", "jobId": self.activeJobId, "progress": e})
}

function OnDispatchWorkerMessage(t) {
    const s = t.data, r = s["type"];
    if ("_import_scripts" === r) importScripts(...s["scripts"]); else if ("_send_blob" === r) self.sentBlobs.set(s["id"], s["blob"]); else if ("_send_buffer" === r) self.sentBuffers.set(s["id"], s["buffer"]); else if ("_ready" === r) SendReady(); else {
        const o = s["jobId"], n = s["isBroadcast"], a = s["params"];
        let e;
