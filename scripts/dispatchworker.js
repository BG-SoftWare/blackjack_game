...
class JobWorker {
    ...
    MaybeStartNextJob() {
        if (!this._isBusy && this._isReady) {
            ...
            if (-1 !== e) {
                const s = self.jobQueue[e], t = s["isBroadcast"];
...
function CancelJob(t) {
    for (let e = 0, s = self.jobQueue.length; e < s; ++e) if (self.jobQueue[e].jobId === t) return void self.jobQueue.splice(e, 1)
}

function OnInputPortMessage(e) {
    const s = e.data, t = s["type"];
    if ("_cancel" === t) CancelJob(s.jobId); else if ("_import_scripts" === t) {
        const o = s["scripts"];
        for (const r of self.jobWorkers) r.ImportScripts(o);
        void self.importedScripts.push(o)
    } else if ("_send_blob" === t) {
        const n = s["blob"], a = s["id"];
        for (const i of self.jobWorkers) i.SendBlob(n, a);
        void self.sentBlobs.push([n, a])
    } else if ("_send_buffer" === t) {
        const f = s["buffer"], l = s["id"];
        for (const u of self.jobWorkers) u.SendBuffer(f, l);
        void self.sentBuffers.push([f, l])
    } else if ("_no_more_workers" === t) self.sentBlobs.length = 0, self.sentBuffers.length = 0, self.importedScripts.length = 0, self.lastBroadcasts.clear(); else {
        if (self.jobQueue.push(s), s["isBroadcast"]) {
            const b = s["maxWorkerNum"], d = self.jobWorkers.length, p = "number" == typeof b ? Math.min(b, d) : d,
